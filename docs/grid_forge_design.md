...
### 3.3 코드 반영 파이프라인: DB → 컴파일 → 샌드박스 무중단 교체
...
5. 서버: DB 저장 + 파일 캐시 쓰기                           (~5ms)
   ...
   │  src/screens/{projectId}/{filename}
   │
6. 서버: esbuild 컴파일                                     (~10ms)
   │  TSX → JS (ES module)
   │
7. 서버: WebSocket 브로드캐스트
   │  { type: "screen_updated", projectId, slug }
   │
8. 브라우저: iframe에 새 번들 로드                            (~50ms)
   │  postMessage로 컴포넌트 교체 알림
   │
9. iframe: 새 컴포넌트 렌더링 → 즉시 반영
   │
   총 지연: LLM 응답 시간 + ~200ms
```
...
[기존 화면 v1 표시 중]
                    ...
                    │
                    │ iframe에 새 번들 URL 전달 (postMessage)
                    │ v1은 계속 표시됨 ← 끊김 없음
                    │
                    │ ......로딩 중......
                    │
                    │ iframe 내부에서 새 컴포넌트 렌더링 완료
                    │ 부모에게 "ready" 메시지
                    │
...
### 3.4 런타임 샌드박스: iframe 격리

AI가 생성한 코드는 **샌드박스 iframe 안에서 실행**된다. 정규식 기반 정적 분석만으로는 보안을 보장할 수 없으므로, **브라우저가 격리를 강제**한다.
...
│  │       script-src 'self'                        │  │
│  │                                                │  │
│  │  Mini React Runtime + AI 생성 컴포넌트          │  │
│  │                                                │  │
│  │  외부 네트워크 접근 불가 (브라우저 강제)          │  │
│  │  부모 DOM 접근 불가 (sandbox 강제)              │  │
│  │  eval/Function 생성 불가 (CSP 강제)             │  │
│  │                                                │  │
│  └────────────────────────────────────────────────┘  │
│           ▲                    │                      │
│           │   postMessage      │                      │
│      tags/state 전달      이벤트(setTags 등) 전달     │
│                                                      │
└──────────────────────────────────────────────────────┘
```

**postMessage 프로토콜 (델타 업데이트):**

대량의 태그를 매번 전체 전송하면 직렬화 비용이 크므로, **최초 전체 동기화 + 이후 변경분만 전송**하는 델타 방식을 사용한다.
...
Parent → iframe:
  ...
  { type: "tagDelta",
    changes: {              // 변경된 태그만
      "/tr1/voltage": { v: 221, q: "good", t: 1709... },
      "/cb1/status": { v: 1, q: "good", t: 1709... }
    } }

  알람 이벤트:
  { type: "alarmUpdate", event: { ... } }

  번들 교체:
  { type: "load", bundleUrl: "/api/screens/{projectId}/{slug}/bundle.js" }

iframe → Parent:
  { type: "setTags", pairs: { "/site1/cb1/command": 1 } }
  { type: "setState", patch: { targetVoltage: 225 } }
  { type: "queryChart", requestId: "...", tagId: "...", from: ..., to: ... }
  { type: "navigate", screenSlug: "overview" }
  { type: "ready" }

Parent → iframe (응답):
  { type: "chartResult", requestId: "...", data: [...] }
```

**성능 근거:**

```text
전체 동기화 (init): 500태그 × ~60B = ~30KB (1회)
델타 업데이트: 변경 태그 ~20개 × ~60B = ~1.2KB (100ms마다)
→ postMessage + structuredClone 비용: < 0.5ms
```
...
| 위협 | 방어 수단 | 동작 |
|------|-----------|------|
| `fetch`/`XMLHttpRequest` | CSP `connect-src 'none'` | 브라우저가 차단 |
| `import` 외부 모듈 | iframe 격리 + 번들에 미포함 | 로드 실패 |
| `document`/`window` 부모 접근 | `sandbox` 속성 | cross-origin 차단 |
| `eval`/`Function` 동적 실행 | CSP `script-src` | 브라우저가 차단 |
| `localStorage`/`cookie` 접근 | sandbox no same-origin | 접근 불가 |

### 3.5 런타임 상태 보존: 코드는 껍데기, 데이터는 바깥

코드가 교체되어도 **런타임 상태(실시간 데이터, 알람, 차트, 사용자 입력값)는 유지**되어야 한다.

**핵심 원칙:** 화면 코드는 렌더링 로직만 담당한다. 모든 런타임 상태는 `ScreenRenderer`(부모)가 관리하고 postMessage로 iframe에 주입하므로, iframe 내부 컴포넌트가 교체되어도 데이터는 그대로 살아있다.

```text
ScreenRenderer (부모 — 불변, 코드 교체에 영향 없음)
│
│  아래 상태들은 ScreenRenderer의 hooks가 관리
│  iframe 컴포넌트가 교체되어도 hooks는 재실행되지 않음
│
├── tags              실시간 태그 값 (WebSocket 구독)
├── setTags           제어 명령 전송 (iframe → postMessage → 서버)
├── alarms            알람 이력
├── chartData         차트에 로드된 시계열 데이터
├── userSettings      사용자 설정값 (테마, 필터 등)
└── screenState       화면별 사용자 입력값 (폼 필드, 선택값 등)
    │
    │  postMessage (init → tagDelta)
    ▼
  iframe 내 Screen 컴포넌트 (교체 대상 — 수신한 데이터로 렌더링만 수행)
```
...
AI가 코드 수정 → DB 저장 → 컴파일 → WebSocket 알림
  ...
  ▼
ScreenRenderer (부모):
  1. iframe에 새 번들 URL 전달 (postMessage: load)
  2. iframe이 새 컴포넌트 로드 + "ready" 알림
  3. 부모가 현재 상태를 postMessage: init로 전달
     │
     ├── tags: 그대로 (WebSocket 구독 유지)
     ├── alarms: 그대로 (30건 유지)
...
iframe 내 화면 코드에서 사용할 수 있는 props (postMessage를 통해 수신):
...
**금지 패턴 (iframe 샌드박스가 강제하되, 코드 품질 피드백용으로도 검증):**

- `useState`, `useReducer` — `screenState` 사용 (코드 교체 시 내부 state 소실 방지)
- `useEffect`로 데이터 fetch — `chartData.query` 사용

위 패턴은 AST 분석으로 검출하여 LLM에 재생성 요청한다. `import`, `fetch`, `eval` 등 보안 관련 패턴은 iframe CSP가 강제 차단하므로 정적 분석은 품질 피드백 목적으로만 수행한다.
...
검증 역할:
  보안 → iframe sandbox + CSP가 담당 (브라우저 레벨)
  품질 → AST 분석이 담당 (LLM 피드백용)
...
│  ├────────────────────────────────┤  │
│  │     iframe (ScreenRenderer)    │  │  z-index: 아래
...
- **모니터링**: 태그 값 변경 → TagBus 콜백 → Bridge WebSocket → Next.js → 브라우저 (tagDelta)
- **제어**: iframe `setTags()` → postMessage → Next.js → 서버 검증 (권한 + rate limit) → Bridge → `tb.set_tags()` → Edge Gateway → 설비
...
Time-Series DB (외부 서버)
    │ 쿼리 결과
    ▼
Next.js → postMessage: chartResult → iframe 렌더링
```
...
```text
iframe postMessage: setTags({ '/acme/plant1/cb1/command': 1 })
    │
    ▼
ScreenRenderer (부모): postMessage 수신
    │  클라이언트 선제 차단 (UX 목적, 우회 가능하므로 보안용 아님)
    │
    ▼
Next.js API (서버 — 모든 검증은 서버 사이드에서 강제):
    │
    ▼
1. 인증 확인 (JWT/세션)
    │
...
사용자 버튼 클릭 (iframe 내부)
    │
    │  postMessage: setTags({ '/site1/cb1/command': 1 })
    ▼
ScreenRenderer (부모)
    │
    │  WebSocket 전송
...
현장 설비 (차단기 투입)
    ...
    │  상태 변경 피드백
    ▼
Edge Gateway → TagBus → Bridge → Next.js → 부모 (tagDelta) → iframe
    │
...
| Preview | 생성된 화면 실시간 미리보기 (iframe). [캡처] 버튼으로 스크린샷 다운로드 |
